...
Future<http.Response?> generalPost({required String url, required Map<String, dynamic> body}) async {
  var response = await http.post(Uri.parse(url), body: body, headers: ApiServices().defaultHeader);
  if (response.statusCode == 200) {
    return response;
  } else {
    return null;
